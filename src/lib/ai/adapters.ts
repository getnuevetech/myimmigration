...
function safeProviderBase(raw: string, fallback: string): string {
  const candidate = (raw || fallback).replace(/\/$/, "");
  // Block private/link-local targets; allow any https host so self-hosted
  // OpenAI-compatible proxies still work when admin-configured.
  return assertSafeOutboundUrl(candidate).toString().replace(/\/$/, "");
}

async function callOpenAiCompatible(p: AiProvider, messages: ChatMessage[], media: MediaAttachment[] = []): Promise<string> {
  const base = safeProviderBase(p.baseUrl, "https://api.openai.com/v1");
...
async function callAnthropic(p: AiProvider, messages: ChatMessage[], media: MediaAttachment[] = []): Promise<string> {
  const base = safeProviderBase(p.baseUrl, "https://api.anthropic.com");
...
async function callGoogle(p: AiProvider, messages: ChatMessage[], media: MediaAttachment[] = []): Promise<string> {
  const base = safeProviderBase(p.baseUrl, "https://generativelanguage.googleapis.com/v1beta");
  const system = messages.filter((m) => m.role === "system").map((m) => m.content).join("\n");
