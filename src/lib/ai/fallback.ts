...
// discrepancies — grounds notice explanations in the admin-curated USCIS
...
// ---- USCIS transcript reader: parses transaction-code rows and the account
// balance out of transcript text (digital PDFs carry a text layer), turning
// the USCIS's own records into authoritative amounts. ----
...
export async function fallbackAnalyze(
  ...
  // The transcript is the USCIS's own record — its figures are authoritative.
  ...
  if (transcript.refundIssued) {
    ...
    if (receivedRefund && Math.abs(receivedRefund.amount - t.amount) > 1 && !receivedRefund.fromDocument) {
      conflicts.push({
        ...
        description: `You reported receiving approximately ${usd(receivedRefund.amount)}, but your USCIS transcript shows a refund of ${usd(t.amount)} issued on ${t.date}.`,
  ...
  if (transcript.accountBalance !== null && transcript.accountBalance > 0) {
    if (balanceDue && Math.abs(balanceDue.amount - transcript.accountBalance) > 1 && !balanceDue.fromDocument) {
      conflicts.push({
        topic: "Amount owed",
        description: `You reported owing approximately ${usd(balanceDue.amount)}, but your USCIS transcript shows an account balance of ${usd(transcript.accountBalance)}.`,
  ...
  const evidenceGuidance = (year: number | null) => {
    ...
    if (!hasDocs) {
      return {
        what: `To verify the numbers we need two specific documents: your USCIS Account Transcript for ${yr} (downloads instantly from your USCIS online account) and your tax return (Form 1040) for ${yr}.`,
    ...
    if (!hasTranscript) {
      return {
        what: `You've added ${docs.length} document${docs.length === 1 ? "" : "s"} — the missing key piece is your USCIS Account Transcript for ${yr}. It lists code-by-code what the USCIS actually did (refunds issued, credits transferred, holds), which settles the numbers.`,
  ...
  const evidenceLine = () =>
    !hasDocs
      ...
      : hasTranscript || transcript.transactions.length > 0
        ? `${docs.length} document${docs.length === 1 ? "" : "s"} uploaded, including your USCIS Account Transcript — the USCIS's own record of your account.${transcriptDetail()}${unreadableCount > 0 ? ` ${unreadableCount} scanned document${unreadableCount === 1 ? "" : "s"} still require${unreadableCount === 1 ? "s" : ""} verification.` : ""}`
        : `${docs.length} document${docs.length === 1 ? "" : "s"} uploaded${hasReturn ? ", including your tax return" : ""}. The missing keystone is your USCIS Account Transcript — it is the USCIS's own record and settles amounts definitively.`;
  ...
  if (expectedRefund && receivedRefund && expectedRefund.amount > receivedRefund.amount) {
    ...
    // CONFIRMED from the USCIS's own records — no guessing needed.
    ...
    issues.push({
      ...
      what_we_know: offsetConfirmed
        ? `Your expected refund was ${usd(expectedRefund.amount)}; the USCIS issued ${usd(receivedRefund.amount)} (transcript TC 846${transcript.refundIssued ? ` on ${transcript.refundIssued.date}` : ""}) and transferred ${usd(offsetTotal)} to another balance (TC 826 on ${offsetDates}). The difference is fully accounted for by the offset.`
      ...
      our_conclusion: offsetConfirmed
        ...
        : `${usd(diff)} of your ${yearText} refund is unaccounted for. The pattern matches a refund offset, an USCIS adjustment, or a partial hold — your Account Transcript identifies which one within its transaction codes.`,
      still_unclear: offsetConfirmed
        ...
        : [
            ...
            `Whether any related USCIS notice was issued that you haven't received or uploaded`,
          ],
      explanations: offsetConfirmed
        ...
        : [
            ...
            { title: "USCIS adjustment", detail: "The USCIS may have changed the amount of your refund (math corrections or credit changes). You would normally receive a notice such as a CP12.", likelihood: "Possible" },
      ...
      alternative_action: "Have a MyImmigration professional review the case with you.",
      analysis_outline: [
        ...
        { heading: "Tax rules", detail: `Rule: refunds don't simply shrink — under USCIS procedure the difference is an offset (refund applied to another debt), an adjustment (the USCIS changed the return), or a hold (payment suspended for review). Why it matters to your case: each explanation has a different remedy, and your Account Transcript distinguishes them by transaction code (TC 826 credit transferred · CP12-type adjustment notices · TC 570 hold).`, source: "USCIS Account Transcript transaction codes · Treasury Offset Program · CP12 notice guidance" },
        ...
        { heading: "Your next move", detail: offsetConfirmed
            ...
            : hasTranscript ? `Match the transcript's transaction codes against the missing ${usd(diff)} — or have a professional confirm the reading.` : `Download your ${yearText} Account Transcript (instant from your USCIS online account) and add it here — it answers this finding and strengthens every other one in this case.` },
      ],
    });
    ...
    issues.push({
      ...
      explanations: [
        ...
        { title: "USCIS adjustment", detail: "The USCIS corrected the return (math errors, credit changes) and refunded a different amount — a CP12-type notice normally follows.", likelihood: "Possible" },
      ...
      analysis_outline: [
        ...
        { heading: "Tax rules", detail: `Rule: e-filed refunds normally issue within 21 days of acceptance; when the amount differs from the return, USCIS procedure allows exactly three causes — offset (TC 826), adjustment (CP12-type notices), or review hold (TC 570). Why it matters to your case: all three are visible on your Account Transcript, so the exact cause is establishable from one free document.`, source: "USCIS refund timing guidance · Account Transcript transaction codes (TC 846, TC 826, TC 570) · CP12 notice guidance" },
        { heading: "Your evidence", detail: evidenceLine() },
  ...
  if (balanceDue) {
    ...
    issues.push({
      ...
      what_we_know: `${balanceFromTranscript ? "Your USCIS Account Transcript shows an account balance of" : balanceDue.fromDocument ? "Your uploaded records show" : "Your information mentions"} ${usd(balanceDue.amount)}${balanceFromTranscript ? " — the USCIS's own current figure" : " owed to the USCIS"}${primaryYear ? ` for tax year ${primaryYear}` : ""}.${transcript.penalties.length ? ` Of that, ${usd(transcript.penalties.reduce((s, p) => s + Math.abs(p.amount), 0))} is penalties/interest per the transcript — some penalties may be eligible for relief depending on the circumstances.` : ""} Depending on your balance and circumstances, you may have several payment or collection-resolution options, and some penalties may be eligible for relief under applicable USCIS rules.`,
      ...
      still_unclear: [
        ...
        "Whether the USCIS has made a recent adjustment",
      ],
      confidence: "medium",
      priority: "high",
      state: hasTranscript ? "review" : "action_needed",
      next_action: hasTranscript ? "REVIEW" : "GET_TRANSCRIPT",
      alternative_action: "Have a MyImmigration professional evaluate the resolution options with you.",
      analysis_outline: [
        { heading: "Your situation", detail: `${balanceDue.fromDocument ? "Your uploaded records show" : "You reported"} ${usd(balanceDue.amount)} owed to the USCIS${primaryYear ? ` for ${yearText}` : ""}.` },
        { heading: "Tax rules", detail: `Rule: an USCIS balance is made of tax + penalties + interest, and each part is treated differently. Why it matters to your case: installment agreements are defined by law (IRC §6159) with streamlined thresholds, some penalties may be eligible for relief depending on your compliance history, and interest follows the balance — so confirming the composition can change both the amount and the options.`, source: "IRC §6159 · Form 9465 instructions · IRM 20.1.1 (penalty relief)" },
        { heading: "Your evidence", detail: evidenceLine() },
        { heading: "Our conclusion", detail: `The balance is ${balanceDue.fromDocument ? "supported by your records" : "reported but not yet documented"}. Once the Account Transcript and any USCIS notice confirm the amount and its composition, MyImmigration can evaluate the legitimate resolution paths for your circumstances.` },
        { heading: "Your next move", detail: hasTranscript ? `Your transcript is on file — confirm the tax/penalty/interest split from its codes, then use the Form 9465 wizard to prepare a payment plan request if needed.` : `Add your ${yearText} Account Transcript and the USCIS notice showing the balance — together they confirm the exact amount so the resolution can be sized correctly.` },
      ],
    });
    ...
    issues.push({
      ...
      what_we_know: `We found evidence that you may have an USCIS balance${primaryYear ? ` for tax year ${primaryYear}` : ""}, but the available information does not yet establish the current amount.${hasDocs ? ` ${docs.length} document${docs.length === 1 ? " is" : "s are"} on file with information relevant to the balance.` : ""} The amount matters because the resolution thresholds are specific: balances of $50,000 or less generally qualify for a streamlined monthly installment agreement (no financial statement), and up to $100,000 can get a 180-day short-term plan.`,
      our_conclusion: "A balance may exist, but until the amount and its composition (tax vs. penalties vs. interest) are established, no resolution option can responsibly be recommended. One number from your USCIS notice or online account — given in the questions below — moves this forward immediately.",
      still_unclear: [
      ...
      explanations: [
        ...
        { title: "USCIS adjustment or proposed change", detail: "A CP2000-type proposal or correction — these can be agreed with, partially agreed, or disputed in writing before the deadline.", likelihood: "Possible" },
      ...
      analysis_outline: [
        { heading: "Your situation", detail: "You reported owing the USCIS, but the exact amount wasn't stated — so the right resolution path can't be sized yet." },
        { heading: "Tax rules", detail: "Rule: resolution options are amount-driven — full payment, streamlined installment agreements under IRC §6159 (≤ $50,000), 180-day plans (≤ $100,000), or hardship status. Why it matters to your case: the facts must be established before the right option can be identified — deciding the solution before the facts is how applicants end up on the wrong plan.", source: "IRC §6159 · Form 9465 instructions · USCIS collection procedures" },
        { heading: "Your evidence", detail: evidenceLine() },
        { heading: "Our conclusion", detail: "The concern is credible but unverified. The USCIS notice (or your online account balance) establishes the amount; the Account Transcript establishes its composition — together they let MyImmigration evaluate the legitimate resolution paths for your circumstances." },
  ...
  for (const code of noticeCodes) {
    ...
    issues.push({
      ...
      our_conclusion: urgent
        ...
        : `Notice ${code} defines what the USCIS wants and by when. ${hasNoticeDoc ? "The notice is on file; its printed deadline and amount govern the response." : "Uploading the notice pins down the deadline and amount that govern the response."}`,
      still_unclear: [
      ...
      analysis_outline: [
        { heading: "Your situation", detail: `You referenced USCIS notice ${code}${primaryYear ? ` for ${yearText}` : ""}. Notice types define exactly what the USCIS wants and by when — identifying the code already tells us most of the story.` },
  ...
  if (/(penalt|interest)/.test(lower)) {
    issues.push({
      ...
      what_we_know: "Your situation mentions penalties or interest. Some penalties may be eligible for relief depending on the circumstances and applicable USCIS rules — first-time abatement applies when the prior three years are penalty-clean, and reasonable-cause relief is a separate path. Relief is requested in writing; we can draft the request letter for you.",
      ...
      alternative_action: "Have a MyImmigration professional assess your eligibility.",
      analysis_outline: [
  ...
  if (/(didn'?t file|not filed|unfiled|late filing|missed filing|never filed)/.test(lower)) {
    issues.push({
      ...
      what_we_know: "Unfiled returns usually must be filed before other resolutions (payment plans, penalty relief) become available. The USCIS may file a 'substitute for return' on your behalf that overstates what you owe.",
      our_conclusion: "Until the unfiled years are identified and filed, most resolution options stay locked — and any USCIS-prepared substitute return likely overstates the true tax.",
      still_unclear: [
        "Which years are unfiled",
        "Whether the USCIS has already filed a substitute return for any year",
        "The income the USCIS has on file for those years (wage & income transcripts show this)",
      ],
      confidence: "medium",
      priority: "high",
      state: "action_needed",
      next_action: "GET_TRANSCRIPT",
      alternative_action: "Have a MyImmigration professional reconstruct the filings with you.",
      analysis_outline: [
        { heading: "Your situation", detail: "Your situation involves one or more unfiled tax returns." },
        { heading: "Tax rules", detail: "Rule: if you don't file, the USCIS may file FOR you — a Substitute for Return (IRC §6020(b)) with the worst assumptions: single status, no deductions, no credits. Why it matters to your case: filing your own accurate return replaces it and usually lowers the bill; USCIS policy generally requires the last 6 years of returns for compliance; and refunds are only payable within 3 years of the due date — late filing can forfeit money you're owed.", source: "IRC §6020(b) · USCIS Policy Statement 5-133 · refund statute of limitations" },
        { heading: "Your evidence", detail: evidenceLine() },
        ...
        { heading: "Your next move", detail: "Get your Wage & Income Transcripts — they list every W-2/1099 the USCIS received, letting you reconstruct income for the unfiled years even if you lost the paperwork." },
      ],
    });
  }
  ...
  if (issues.length === 0) {
    ...
    issues.push({
      ...
      analysis_outline: [
        ...
        { heading: "Tax rules", detail: "Every applicant situation resolves through the same fundamentals: confirm what the USCIS has on file (transcripts), compare it with your records, respond to any notices in writing before their deadlines, and use the defined relief paths where they apply.", source: "USCIS transcript and notice-response procedures" },
        { heading: "Your evidence", detail: evidenceLine() },
  ...
  pathSteps.push({
    ...
    description: hasDocs
      ...
      : "Upload the USCIS notice, your tax return, and any W-2/1099s. Completes automatically when your case has documents.",
    action_key: "UPLOAD_DOCUMENTS",
  });
  if (!hasTranscript) {
    pathSteps.push({
      title: "Get your USCIS account transcript",
      description: "Your transcript shows exactly what the USCIS has on file — including where any missing refund went. Completes when a transcript is in your case documents.",
  ...
  if (noticeCodes.length > 0) {
    pathSteps.push({
      ...
      description: "If you disagree with a notice, the USCIS expects a written response with your supporting documents. Generate a professional reply, edit it, and mail it before the deadline. Completes when a letter exists.",
  ...
  if (/(penalt|interest)/.test(lower)) {
    pathSteps.push({
      ...
      description: "If you're eligible, relief is requested with a short letter (we draft it) or by calling the USCIS. Send it together with any response form from your notice.",
      action_key: "DRAFT_LETTER",
    });
  }
  ...
  pathSteps.push({
    title: "Confirm the resolution with the USCIS",
    description: "After you've responded or arranged payment, confirm the USCIS updated your account (letter or transcript). Mark this done yourself once confirmed.",
    action_key: "",
  });
