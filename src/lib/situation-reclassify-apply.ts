/**
 * Phase S4 — apply legacy Case → Situation reclassification (preserves audit IDs).
 */
import { db } from "@/lib/db";
import { composeAssistantReply, runConversationIntelligence } from "@/lib/conversation";
import {
  decideLegacyCaseDisposition,
  primaryGovernmentSystem,
  type LegacyCaseLike,
} from "@/lib/situation-reclassify";
import { situationTitleFromNarrative } from "@/lib/situation";

export type ReclassifyApplyResult = {
  scanned: number;
  kept: number;
  reclassified: number;
  decisions: ReturnType<typeof decideLegacyCaseDisposition>[];
};

export async function scanLegacyCasesForReclassify(limit = 500): Promise<ReclassifyApplyResult> {
  const rows = await db.case.findMany({
    where: { situationId: null },
    take: limit,
    orderBy: { createdAt: "asc" },
    select: {
      id: true,
      number: true,
      title: true,
      situation: true,
      goal: true,
      userId: true,
      guestSessionId: true,
      intelligenceJson: true,
      notices: { select: { noticeType: true } },
      documents: { select: { fileName: true }, take: 10 },
    },
  });

  const decisions = rows.map((row) =>
    decideLegacyCaseDisposition({
      ...row,
      documentHints: row.documents.map((d) => d.fileName),
    }),
  );

  return {
    scanned: rows.length,
    kept: decisions.filter((d) => d.action === "keep_case").length,
    reclassified: decisions.filter((d) => d.action === "reclassify_to_situation").length,
    decisions,
  };
}

/**
 * Reclassify situation-only Cases into Situation rows.
 * Kept Cases get governmentSystem stamped when missing.
 */
export async function applyLegacyCaseReclassification(opts?: {
  limit?: number;
  dryRun?: boolean;
}): Promise<ReclassifyApplyResult> {
  const dryRun = opts?.dryRun ?? true;
  const limit = opts?.limit ?? 500;

  const rows = await db.case.findMany({
    where: { situationId: null },
    take: limit,
    orderBy: { createdAt: "asc" },
    select: {
      id: true,
      number: true,
      title: true,
      situation: true,
      goal: true,
      userId: true,
      guestSessionId: true,
      intelligenceJson: true,
      governmentSystem: true,
      notices: { select: { noticeType: true } },
      documents: { select: { fileName: true }, take: 10 },
    },
  });

  const decisions = [];
  let kept = 0;
  let reclassified = 0;

  for (const row of rows) {
    const decision = decideLegacyCaseDisposition({
      ...row,
      documentHints: row.documents.map((d) => d.fileName),
    } as LegacyCaseLike);
    decisions.push(decision);

    if (decision.action === "keep_case") {
      kept += 1;
      if (!dryRun && !row.governmentSystem) {
        await db.case.update({
          where: { id: row.id },
          data: { governmentSystem: primaryGovernmentSystem(decision.governmentSystems) || "uscis" },
        });
      }
      continue;
    }

    reclassified += 1;
    if (dryRun) continue;

    const intel = runConversationIntelligence({
      message: row.situation,
      goal: row.goal,
    });
    // Force Situation workspace for migrated options narratives.
    const reply = composeAssistantReply(intel, row.situation);

    const situation = await db.situation.create({
      data: {
        userId: row.userId,
        guestSessionId: row.guestSessionId,
        title: situationTitleFromNarrative(row.situation || row.title, intel.question_contract.explicit_question),
        originalNarrative: row.situation || row.title,
        goal: row.goal,
        questionContractJson: JSON.stringify(intel.question_contract),
        currentDecisionTarget: intel.question_contract.decision_target,
        knownFactsJson: "[]",
        currentPathwaysJson: JSON.stringify(
          intel.strategy.branches.map((b) => ({ id: b.id, condition: b.condition, explanation: b.explanation })),
        ),
        currentRisksJson: "[]",
        status: "guiding",
        intelligenceJson: JSON.stringify({ ...intel, route: { ...intel.route, workspace: "situation" } }),
        learningEventJson: JSON.stringify(intel.experience_record ?? intel.learning_event),
        assistantReply: reply,
        legacyCaseId: row.id,
        legacyRecordType: "case",
        migrationTimestamp: new Date(),
        updatedAt: new Date(),
      },
    });

    // Relink documents that were only on the Case.
    await db.document.updateMany({
      where: { caseId: row.id, situationId: null },
      data: { situationId: situation.id },
    });

    // Soft-close the legacy Case without deleting (audit). Mark link to Situation.
    await db.case.update({
      where: { id: row.id },
      data: {
        situationId: situation.id,
        status: "closed",
        closedReason: "reclassified_to_situation",
        closingRemarks: `Reclassified to Situation ${situation.id} (Phase S4). Legacy IMM retained for audit.`,
        closedAt: new Date(),
      },
    });
  }

  return { scanned: rows.length, kept, reclassified, decisions };
}
