...
import { FilingPlanWorkspaceView } from "@/components/filing-plan-workspace-view";

export default async function FilingPlanPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const user = await requireUser();
  const plan = await db.filingPlan.findFirst({
