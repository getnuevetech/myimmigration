      ...
      <div className="grid gap-6 md:grid-cols-3">
        {plans.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={`relative flex flex-col rounded-2xl border bg-white p-5 shadow-sm ${onSale ? "border-emerald-400 ring-1 ring-emerald-400" : plan.badge ? "border-lime-400 ring-1 ring-lime-400" : "border-slate-200"}`}
            >
              {onSale && (
              ...
              <div className="flex items-center justify-between">
                ...
                {plan.badge && (
                  <span className="rounded-full bg-lime-100 px-2 py-0.5 text-xs font-semibold text-lime-700">{plan.badge}</span>
                )}
              </div>
