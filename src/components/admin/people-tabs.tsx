...
export async function PeopleTabs({ active }: { active: "customers" | "consultants" | "admins" }) {
  ...
  const tabs = [
    ...
    { key: "consultants", href: "/admin/consultants", label: "Immigration Consultants", count: consultants },
  ...
  return (
    <div className="mb-6 flex flex-wrap gap-1 rounded-xl bg-slate-200/70 p-1">
      {tabs.map((t) => (
        ...
        >
          {t.label}
          <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${active === t.key ? "bg-lime-100 text-lime-700" : "bg-slate-300/70 text-slate-600"}`}>
            {t.count}
          </span>
        </Link>
      ))}
