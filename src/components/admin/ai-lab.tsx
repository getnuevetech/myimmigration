      ...
      <div className="space-y-2">
        {value.map((v, i) => (
          <div key={i} className="rounded-lg bg-white p-2.5 ring-1 ring-slate-200">
            <p className="mb-1 text-[10px] font-bold uppercase tracking-wider text-lime-400">#{i + 1}</p>
            <ValueView value={v} depth={depth + 1} />
          </div>
        ))}
      </div>
  ...
  return (
    <div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <form ref={formRef} action={send}>
          <textarea
            ...
            rows={3}
            required
            placeholder='e.g. "I received an RFE for my I-485 and the notice says I must respond within 87 days — what evidence should I organize first?" — attach a case record or notice to test document reading.'
          ...
          <div className="mt-2 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="rounded-lg bg-lime-600 px-5 py-2 text-sm font-semibold text-white hover:bg-lime-700 disabled:opacity-50"
              >
      ...
      <div className="mt-6 space-y-8">
              ...
              {round.results.map((r) => (
                <div key={r.providerId} className={`flex flex-col rounded-2xl border bg-white shadow-sm ${r.ok ? "border-slate-200" : "border-red-300"}`}>
                  <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 px-4 py-2.5">
                    ...
                    {r.visionUsed && <span className="rounded bg-lime-100 px-1.5 py-0.5 text-[10px] font-semibold text-lime-700">read files 👁</span>}
                      ...
                      r.parsed ? (
                        <>
                          ...
                          <details className="mt-3 border-t border-slate-100 pt-2">
                            <summary className="cursor-pointer text-[11px] font-medium text-slate-400 hover:text-lime-600">Raw response (technical)</summary>
        ...
        {pending && (
          <div className="flex items-center gap-3 rounded-2xl border border-lime-200 bg-lime-50 p-6 text-sm text-lime-900">
            <span className="h-3 w-3 animate-ping rounded-full bg-lime-500" />
