...
import { buildFilingPlanContent } from "@/lib/filing-plan";
...
export function FilingPlanWorkspaceView(props: {
  id: string;
  ...
  const fallback = buildFilingPlanContent({ selectedPathway: props.selectedPathway });
  ...
  const requirements = eligibility.requirements?.length ? eligibility.requirements : fallback.eligibility.requirements;
  ...
  return (
    <div className="max-w-3xl space-y-8">
      <header>
        <p className="text-sm font-medium text-lime-700">Filing Plan</p>
        <h1 className="mt-1 text-2xl font-semibold tracking-tight text-slate-900">
          {fallback.pathwayLabel}
        </h1>
        <p className="mt-1 text-sm text-slate-500">
          From {situationRefLabel(props.situation.number)} · Status: {props.preparationStatus}
        </p>
        <p className="mt-2 text-sm text-slate-600">
          <Link href={situationHref} className="font-medium text-lime-700 hover:underline">
      ...
      <section className="space-y-2">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Required filings</h2>
        <ul className="space-y-3">
          {(filings.length ? filings : fallback.filings).map((f) => (
            <li key={`${f.form}-${f.role}`} className="border-l-2 border-lime-500 pl-3">
