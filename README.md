...
A friendly AI immigration assistant that helps people **explore options before they file**, and also understand USCIS notices, matching documents, deadlines, and official questions in plain English — then turns every situation into a simple, step-by-step plan. A USCIS receipt is not required to start. ImmigrationOnMe is **not** USCIS and is **not** a law firm; it is an informational guidance tool with optional referral or handoff to vetted immigration professionals.

## What's in V1

| Feature | Status |
| --- | --- |
| Explore options before a filing (guest-friendly) | ✅ |
| AI immigration Q&A without a receipt number | ✅ |
| Matching USCIS forms, cover letters, and identity/relationship evidence | ✅ |
| Upload / photograph USCIS notices when you already have one | ✅ |
...
| Deadline reminders (only when a notice actually supplies one) | ✅ |
...
1. **Customer input** — situation + goal + documents. Works without an account; a guest session stores everything and attaches it to the user's account on registration. People can start from a life situation with no filing yet, or from a USCIS letter already on file.
...
5. **Verification** — a deterministic consensus engine: agreement merges, disagreement is flagged **"verification required"** (never guessed). Options progress and case readiness (0–100%) are computed by our own formula: matching documents + verified facts + USCIS source confirmation − contradictions. A receipt is not invented for open-options reviews.
...
- **Immigration professionals / consultants** — onboarding for attorneys, accredited representatives, or qualified consultants, including credentials, specialties, languages, and proof uploads. Manual admin approval with optional auto-approval rules. Client assignments require **both** the user's and the consultant's explicit consent before anything is shared. The platform never auto-assigns a professional.
